...
public class HybridMongoDBConstants {
    ...
    public static final String GET_ROLE_LIST_OF_USER_MONGO_QUERY = "{'collection' : 'UM_HYBRID_USER_ROLE',$match : {'UM_USER_NAME' : '?','UM_TENANT_ID' : '?','UM_DOMAIN_NAME' : '?','roles.UM_TENANT_ID' : '?','roles.UM_DOMAIN_ID' : '?'},'$lookup' : {'from' : 'UM_HYBRID_ROLE','localField' : 'UM_ROLE_ID','foreignField' : 'UM_ID','as' : 'roles'},'$unwind' : {path : '$roles','preserveNullAndEmptyArrays' : false},'$project' : {'roles.UM_ROLE_NAME' : 1,_id : 0}}";
